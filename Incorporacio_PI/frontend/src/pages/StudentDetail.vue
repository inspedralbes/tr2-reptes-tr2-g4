      ...
      <v-list lines="two">
        <v-list-item v-for="(file, index) in normalizedFiles" :key="index">
          ...
          <template v-slot:append>
            ...
            <v-btn :href="`http://localhost:4002/uploads/${file.filename}`" target="_blank" icon="mdi-open-in-new"
...
const downloadFile = async (filename, originalName) => {
  try {
    const response = await fetch(`http://localhost:4002/uploads/${filename}`);
...
const generateGlobalSummary = async () => {
    try {
        ...
        const response = await fetch('http://localhost:4002/api/generate-global-summary', {
            method: 'POST',
